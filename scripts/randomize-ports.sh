...
update_env() {
  ...
  if [[ -f "$file" ]]; then
    sed -i.bak "s/^NGINX_HOST_PORT=.*/NGINX_HOST_PORT=$nginx_port/" "$file" && rm -f "$file.bak"
    sed -i.bak "s/^FRONTEND_HOST_PORT=.*/FRONTEND_HOST_PORT=$frontend_port/" "$file" && rm -f "$file.bak"
    echo "  updated $file"
  fi
}
...
update_compose_defaults() {
  local file="$1" nginx_port="$2" frontend_port="${3:-}"
  if [[ -f "$file" ]]; then
    sed -i.bak "s/\${NGINX_HOST_PORT:-[0-9]\+}/\${NGINX_HOST_PORT:-$nginx_port}/g" "$file" && rm -f "$file.bak"
    if [[ -n "$frontend_port" ]]; then
      sed -i.bak "s/\${FRONTEND_HOST_PORT:-[0-9]\+}/\${FRONTEND_HOST_PORT:-$frontend_port}/g" "$file" && rm -f "$file.bak"
    fi
    echo "  updated $file"
  fi
}
